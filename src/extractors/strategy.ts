...
export class StrategyExtractor {
  ...
  async openSettings(strategyIndex: number): Promise<StrategySettings | null> {
    const strategyContainer = document.querySelector(
      SELECTORS.strategies.container
    );
    if (!strategyContainer) return null;

    const strategyItems = strategyContainer.querySelectorAll(
      SELECTORS.strategies.items
    );
    ...
    for (const selector of buttonSelectors) {
      settingsButton = strategyItem.querySelector(
        selector
  ...
  private async extractDialogSettings(dialog: Element): Promise<StrategySettings | null> {
    const dialogTitle = dialog.querySelector(SELECTORS.settingsDialog.title)?.textContent?.trim() || "Unknown Strategy";
    ...
    labels.forEach((label: Element, index: number) => {
      ...
      if (valueElement instanceof HTMLInputElement) {
        value = valueElement.type === "checkbox" 
          ? (valueElement.checked ? "true" : "false")
    ...
    const closeButton = dialog.querySelector(SELECTORS.settingsDialog.closeButton) as HTMLButtonElement ||
